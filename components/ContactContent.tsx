...
import { ADDRESS, EMAIL, GEO, PHONE_DISPLAY, PHONE_TEL, SOCIAL_LINKS } from "@/lib/data";
...
const MAPS_EMBED_SRC = `https://maps.google.com/maps?q=${GEO.latitude},${GEO.longitude}&z=16&output=embed`;
    ...
    <div className="mx-auto max-w-[1440px] px-6 pb-24 pt-36 md:px-10 md:pt-44">
      <div className="grid grid-cols-1 gap-16 md:grid-cols-2 md:gap-24">
        <div>
          ...
          <div className="mt-12 flex flex-col gap-2 border-t border-line pt-8">
            <span className={labelClass}>{page.addressLabel}</span>
            <p className="max-w-sm text-sm leading-relaxed text-ink/70">
              {ADDRESS.full}
            </p>
            <p className="mt-2 font-mono text-[11px] uppercase tracking-[0.1em] text-mist">
              {page.hoursLabel}: {page.hoursValue}
            </p>
          </div>
      ...
      <div className="mt-24 border-t border-line pt-12 md:mt-32">
        <div className="relative aspect-[16/9] w-full overflow-hidden bg-cloud md:aspect-[21/9]">
          <iframe
            src={MAPS_EMBED_SRC}
            title={`${ADDRESS.full} — Google Maps`}
            loading="lazy"
